...
/**
 * <p>
 * This action will get a contest with the given id. This action requires
 * <code>projectId</code> or <code>contestId</code> parameter (mutual exclusive)
 * be present for each call.
 * </p>
 * <p>
 * <b>Thread Safety</b>: In <b>Struts 2</b> framework, the action is constructed
 * for every request so the thread safety is not required (instead in Struts 1
 * the thread safety is required because the action instances are reused). This
 * class is mutable and stateful: it's not thread safe.
 * </p>
 * <p>
 * Version 1.1 - Direct - View/Edit/Activate Studio Contests Assembly Change
 * Note
 * <ul>
 ...
 * <ul>
 * <li>Updated {@link #executeAction()} method to retrieve data for contest
 ...
 * <ul>
 * <li>Updated {@link #executeAction()} method to retrieve data for contest
 * dashboard section use non-cached model.</li>
 * </ul>
 * <p>
 ...
 * <ul>
 * <li>Updated {@link #executeAction()} method to set hasContestWritePermission
 * flag to the view data.</li>
 * </ul>
 * </p>
 * <p>
 * Version 1.2.5 - TC Cockpit Bug Tracking R1 Cockpit Project Tracking version
 * 1.0 Change Note
 * <ul>
 * <li>Updated {@link #executeAction()} method to set UnresolvedIssuesNumber to
 * the dashboard view data.</li>
 * </ul>
 * </p>
 *
 * <p>
 ...
 * <ul>
 * <li>Remove contestId and studioCompetition fields and getter/setter for them
 * because this studio contest is the same as software contest now.</li>
 * <li>Update {@link #executeAction()} method to only handle the
 * <code>SoftwareCompetition</code>.</li>
 * <li>Update PrizeSortByPlace to sort <code>Prize</code>.</li>
 * <li>Update {@link #isSoftware()} method to use the new logic to determine
 * whether a contest is a software contest or not.</li>
 * <li>Update {@link #getViewData()} method to only handle the
 * <code>SoftwareCompetition</code>.</li>
 * <li>Remove fillContestStats method because we are using
 * DirectUtils.getContestStats method instead.</li>
 * <li>Update {@link #getSessionData()} method to only handle the
 * <code>SoftwareCompetition</code>.</li>
 * </ul>
 * </p>
 * <p>
 ...
 * <ul>
 * <li>Update {@link #isSoftware()} method to support the copilot contest as a
 * software contest.</li>
 * <li>Update {@link #executeAction()} method to set the contest type to studio
 * if contestStats.getIsStudio returns true.</li>
 * </ul>
 * </p>
 * <p>
 * Version 1.5 - BUGR-6609 Change Note
 * <ul>
 * <li>Added {@link #subEndDate} and {@link #contestEndDate} fields. Also the
 * getters were added.</li>
 * </ul>
 * </p>
 * <p>
 * Version 1.6 - Release Assembly - TC Direct Cockpit Release Four change note
 * <ol>
 * <li>Update {@link #getProjectName(long)} to use project service to retrieve
 * project name</li>
 * </ol>
 * </p>
 *
 * <p>
 * Version 1.7 (Release Assembly - TopCoder Cockpit Software Checkpoint
 * Management) Change notes:
 * <ol>
 * <li>Updated {@link #executeAction()} method to add parameter
 * softwareCompetition when calling updated method
 * {@link DirectUtils#getContestStats(TCSubject, long, SoftwareCompetition)}.</li>
 * </ol>
 * </p>
 *
 * <p>
 * Version 1.8 (Cockpit Customer Copilot Posting Process Revamp Copilot Posting
 * Dashboard) change notes:
 * <ol>
 * <li>Refactor the copilot posting specific data into the contest dashboard
 * data</li>
 * </ol>
 * </p>
 *
 * <p>
 * Version 1.9 (Release Assembly - TC Cockpit Enterprise Dashboard Project
 * Pipeline and Project Completion Date Update)
 * <ol>
 * <li>Add check null for current select project id in session data.</li>
 * </ol>
 * </p>
 *
 * <p>
 ...
 * <ul>
 * <li>Adds the billing accounts of the project which the contest is in into
 * action</li>
 * </ul>
 * </p>
 *
 * <p>
 * Version 2.1 (Release Assembly - TopCoder Cockpit - Marathon Match Contest
 * Detail Page)
 * <ol>
 * <li>Adds the method {@link #isMarathon()} to check whether the contest is of
 * type marathon</li>
 * </ol>
 * </p>
 *
 * <p>
 * Version 2.2 BUGR-8788 (TC Cockpit - New Client Billing Config Type) change
 * notes:
 * <ul>
 * <li>change on {@link #billingAccountsForProject} to suuport CCA related for
 * billing account</li>
 * </ul>
 * </p>
 *
 * <p>
 * Version 2.3 (PoC Assembly - TopCoder Cockpit - Tracking Marathon Matches
 * Progress) change notes:
 * <ul>
 * <li>Add property {@link #marathonMatchAnalyticsService} to support marathon
 * match contest.</li>
 * <li>Update method {@link #executeAction()} to support marathon match
 * contest.</li>
 ...
 *
 * <p>
 * Version 2.4 (Release Assembly - TopCoder Cockpit - Tracking Marathon Matches
 * Progress - Dashboard and Submissions Tab) change notes:
 * <ul>
 * <li>Add property {@link #active}.</li>
 * <li>Update method {@link #executeAction()} to support marathon match contest
 * dashboard part.</li>
 * </ul>
 * </p>
 *
 * <p>
 * Version 2.5 (Release Assembly - TopCoder Cockpit - Tracking Marathon Matches
 * Progress - Results Tab) change notes:
 * <ul>
 * <li>Update method {@link #executeAction()} because the api of
 * <code>MarathonMatchHelper</code> has been changed.</li>
 ...
 * <li>Updates the method {@link #executeAction()} to set the milestone name for
 * the software competition and pre-populate the project milestones</li>
 * </ul>
 * </p>
 *
 * <p>
 * Version 2.7 (Release Assembly - TC Cockpit New Challenge types Integration
 * Bug Fixes)
 * <ul>
 ...
 *
 * <p>
 * Version 2.8 (TopCoder Direct - Update jira issues retrieval to Ajax) @author
 * -jacob- @challenge 30044583
 * <ul>
 * <li>Updated method {@link #executeAction()} to remove the code to set the
 * unresolved issues number</li>
 * </ul>
 * </p>
 *
 * <p>
 * Version 2.9 (topcoder Direct Refactor Jira RPC and VM Count Retrieval to
 ...
 *         Removes the statements to populate the right sidebar direct projects
 *         and project contests. It's changed to load these data via ajax
 *         instead after the page finishes loading.
 *         </p>
 *
 *         <p>
 *         Version 3.1 (TopCoder Direct - Draft Challenge Creation/Saving
 *         Prompt)
 *         <ul>
 *         <li>Adds the {@link #showSaveChallengeConfirmation} and its
 *         getter</li>
 *         <li>Updates {@link #executeAction()} to populate the
 *         {@link #showSaveChallengeConfirmation}</li>
 *         </ul>
 *         </p>
 *
 *         <p>
 *         Version 3.2 (Topcoder - Ability To Set End Date For Registration
 ...
 * 
 *         <p>
 *         Version 3.3 - Topcoder - Remove JIRA Issues Related Functionality In
 *         Direct App v1.0 - remove JIRA related functionality
 *         </p>
 * 
 *         <p>
 *         Version 3.4 - Quick72Hrs!! Topcoder - Remove VM Management Feature In
 *         Direct App version 1.0 - remove VM related functionality
 *         </p>
 *
 *         <p>
 *         Version 3.5 - Topcoder - Add effort hours field - add enable effort
 *         hours
 *         </p>
 *
 ...
 *         Ghost_141, GreatKevin, Veve, GreatKevin, TCSCODER
 * @version 3.5
 */
public class GetContestAction extends ContestAction {
   ...
   * </p>
   * <p>
   * It's used to retrieve the software competition. It can be 0 (it means not
   * present) or greater than 0 if it's present. It's changed by the setter and
  ...
  /**
   * <p>
   * view data. It is copied from old details page to preserve some portion of the
   * existing page.
   * </p>
   */
  ...
  /**
   * <p>
   * Session data. It is copied from old details page to preserve some portion of
   * the existing page.
   * </p>
   */
  ...
  protected void executeAction() throws Exception {
    ...
    // TODO: this is TEMPORARY solution to eliminate same call for contest and
    // contest json
    ...
    if (softwareCompetition == null) {
      softwareCompetition = contestServiceFacade
          .getSoftwareContestByProjectId(DirectStrutsActionsHelper.getTCSubjectFromSession(), projectId);
    ...
    softwareCompetition.getProjectHeader()
        .setGroups(DirectUtils.getGroupIdAndName(softwareCompetition.getProjectHeader().getGroups()));
    ...
    DirectUtils.setDashboardData(currentUser, projectId, getViewData(), getContestServiceFacade(),
        !DirectUtils.isStudio(softwareCompetition));
    ...
    if (DirectUtils.isMM(softwareCompetition)) {
      ...
      if (roundId != null && !roundId.equals("") && NumberUtils.isNumber(roundId)) {
        ...
        // MarathonMatchHelper.getMarathonMatchDetails(roundId,
        // marathonMatchAnalyticsService, timelineInterval,
        // viewData);
      } else {
        hasRoundId = false;
      }
    ...
    if (softwareCompetition.getProjectData().getContestSales() != null
      ...
      contestStats
          .setPaymentReferenceId(softwareCompetition.getProjectData().getContestSales().get(0).getSaleReferenceId());
    }
    ...
    if (prizes != null) {
      ...
      // set copilots
      this.copilots = DataProvider
          .getCopilotsForDirectProject(softwareCompetition.getProjectHeader().getTcDirectProjectId());

      this.copilotCost = calculateCopilotCost(softwareCompetition.getResources());

      // set contest permission
      viewData.setHasContestWritePermission(DirectUtils.hasWritePermission(this, currentUser, projectId, false));
    ...
    this.milestones = getMilestoneService().getAll(softwareCompetition.getProjectHeader().getTcDirectProjectId(),
        Arrays.asList(MilestoneStatus.values()), SortOrder.ASCENDING);
    ...
    if (softwareCompetition.getProjectHeader().getProjectCategory().getId() == 29) {
      DirectUtils.setCopilotDashboardSpecificData(getProjectServices(), getProjectServiceFacade(), getMetadataService(),
          projectId, softwareCompetition.getProjectHeader().getTcDirectProjectId(), getViewData().getDashboard());
    }

    // set billing accounts for the direct project
    List<Project> billingProjects = getProjectServiceFacade()
        .getBillingAccountsByProject(softwareCompetition.getProjectHeader().getTcDirectProjectId());
    ...
    try {
      String value = userPreference.getValue(DirectUtils.getTCSubjectFromSession().getUserId(),
          ChallengeConfirmationPreferenceAction.CHALLENGE_CONFIRMATION_PREFERENCE_ID, DBMS.COMMON_OLTP_DATASOURCE_NAME);
  ...
  public String specReviewScheduled() {
    try {
      ...
      final Submission[] activeSpecSubmission = getContestServiceFacade().getSoftwareActiveSubmissions(getProjectId(),
          2);
  ...
  /**
   * The <code>Comparator</code> to compare two <code>Prize</code> object ordered
   * by the prize place.
   *
   * @author TCSASSEMBER
   */
  ...
  /**
   * <p>
   * Gets project name. NOTE: it is fixing some bug which software competition
   * project header is missing project name population.
   * </p>
   *
   * <p>
   ...
   * </p>
   *
   * @return a <code>Map</code> mapping the project copilot type ids to category
   *         names.
