...
$(document).ready(function(){
	...
	function setFieldErrorForRegistration(fieldName, errorFieldName, errorMessage) {
		$(fieldName).parent().parent().find('label').addClass('errorLabel');
	...
	$('#submitButton').live('click',function(){
		...
		if (firstName == '') {
			flag = 0;
			setFieldErrorForRegistration('#firstName', '.firstNameErr', 'Please fill in your first name');
		}
		var lastName = $.trim($('#lastName').val());
		if (lastName == '') {
			flag = 0;
			setFieldErrorForRegistration('#lastName', '.lastNameErr', 'Please fill in your last name');
		}
		var handle = $.trim($('#handle').val());
		if (handle == '') {
			flag = 0;
			setFieldErrorForRegistration('#handle', '.handleErr', 'Please fill in your desired handle');
		}
		var email = $.trim($('#email').val());
		if (email == '' || !$('#email').val().match(/^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+((\.[a-zA-Z0-9_-]{2,3}){1,2})$/)) {
			flag = 0;
			setFieldErrorForRegistration('#email', '.emailErr', 'Please fill in your email');
		}
		var password = $.trim($('#passwordWord').val()); 
		if (password == '') {
			flag = 0;
			setFieldErrorForRegistration('#passwordWord', '.regPasswordErr', 'Please fill in your password');
		}
		var confirmPassword = $.trim($('#confirmPassword').val());
		if (confirmPassword == '' || password != confirmPassword) {
			flag = 0;
			setFieldErrorForRegistration('#confirmPassword', '.passwordNotMactchErr', 'Password does not match');
		}
		var verCode = $.trim($('#verificationCode').val()); 
		if (verCode == '') {
			flag = 0;
			setFieldErrorForRegistration('#verificationCode', '.veriCodeErr', 'Please enter the verification code');
		}
		...
		if(flag){
			...
			var fieldErrorMap = {'firstName':['firstName','firstNameErr'],'lastName':['lastName','lastNameErr'],'handle':['handle','handleErr'],
					'email':['email','emailErr'],'regPassword':['passwordWord','regPasswordErr'],'veriCode':['verificationCode','veriCodeErr']};
            $.ajax({
                type: 'POST',
                  ...
                  function(result) {
                      if (jsonResult.jsonData.fieldErrors) {
                          ...
                          for (var fname in jsonResult.jsonData.fieldErrors) {
                              ...
                              setFieldErrorForRegistration('#'+fieldErrorMap[fname][0],'.'+fieldErrorMap[fname][1],errorMessages);
                          }
                          if (hasError) {
