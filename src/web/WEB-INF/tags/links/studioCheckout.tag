<%--
  ...
  -
  - Description: This tag renders the HTML markup for A element referencing the page with Studio Submissions Checkout page.
--%>
...
<c:choose>
    <c:when test="${milestoneRound}">
        <a href="<s:url action="checkout" namespace="/contest">
                     <s:param name="contestId" value="%{#attr['contestId']}"/>
                     <s:param name="formData.viewType" value="'CHECKOUT'"/>
