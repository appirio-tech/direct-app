...
GanttProject.prototype.checkWidthProjectNameItem = function()
{
    ...
    this.projectNameItem.innerHTML = "<a href=\"" + ganttProjectUrl + '" target="_blank">' + pName + "</a>";
};
/**
...
GanttChart.prototype.getXML = function()
{
    ...
    for (var i = 0; i < this.Project.length; i++)
    {
        ...
        for (var j = 0; j < this.Project[i].ParentTasks.length; j++)
        {
            ...
            strXML += "<est>" + (this.Project[i].ParentTasks[j].EST.getMonth() + 1) + "," + this.Project[i].ParentTasks[j].EST.getFullYear() + "," + this.Project[i].ParentTasks[j].EST.getDate() + "</est>";
...
GanttChart.prototype.createPanelTasks = function()
{
    ...
    if (divTasks.style.height != undefined && !($.browser.msie && $.browser.version=="7.0")) {
...
GanttChart.prototype.createPanelNamesTasks = function()
{
    ...
    if (divListNames.style.height != undefined && !($.browser.msie && $.browser.version == "7.0")) {
...
GanttChart.prototype.getPrintableHTML = function()
{
    ...
    var res = '<html><head><link type="text/css" rel="stylesheet" href="'+this.stylePath+'"><scr'+'ipt>onload=function(){var w=' + w + ',h=' + h +
            ...
            'c2.style.width=parseInt(c2.style.width)+w+300+"px";c2.previousSibling.style.width=c2.style.width;c1.style.width=parseInt(c1.style.width)+w+"px";c2.style.height=parseInt(c2.style.height)+h+"px";' +
...
GanttTask.prototype.getPopUpInfo = function(object, event)
{
    ...
    tblInfo.rows[0].cells[0].innerHTML += "<span class='st'>Start Date:&nbsp;</span><span class='ut'>" + (this.TaskInfo.EST.getMonth() + 1) + "." + this.TaskInfo.EST.getDate() + "." + this.TaskInfo.EST.getFullYear() + "</span><br/>";
    tblInfo.rows[0].cells[0].innerHTML += "<span class='st'>Duration:&nbsp;</span><span class='ut'>" + (this.TaskInfo.Duration/24.0).toFixed(1) + " days </span><br/>";
...
GanttProject.prototype.getPopUpInfo = function(object, event)
{
    ...
    tblInfo.rows[0].cells[0].innerHTML += "<span class='st'>Duration:&nbsp;</span><span class='ut'>" + (this.Duration/24.0).toFixed(1) + " days</span><br/>";
...
GanttProject.prototype.getDescStr = function()
{
    ...
    for (var i = 0; i < this.Chart.paramShowProject.length; i++) {
        ...
        switch (this.Chart.paramShowProject[i]) {
            ...
            case "Duration":
                if (str != "")str += delim;
                str += (this[this.Chart.paramShowProject[i]] / 24) + " days";
                break;
...
GanttTask.prototype.getDescStr = function()
{
    ...
    for (var i = 0; i < this.Chart.paramShowTask.length; i++) {
        ...
        switch (prop) {
            ...
            case "Duration":
                if (str != "")str += delim;
                str += (propValue/24).toFixed(1) + " days";
                break;
            case "PercentCompleted":
                if (str != "")str += delim;
                str += propValue + "%";
    ...
    if (this.showObjectInfo)
    {
        if (this.object) {
            if (this.object.constructor == GanttTask) {
                ...
                this.insertData(t, "Start Date", (this.object.TaskInfo.EST.getMonth() + 1) + "." +  this.object.TaskInfo.EST.getDate() + "." + this.object.TaskInfo.EST.getFullYear());
