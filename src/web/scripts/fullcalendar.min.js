			...
			for (i=0; i<rowCnt; i++) {
				...
				for (j=0; j<colCnt; j++) {
					...
					s += "<td class='fc-" +
						...
						' fc-not-today') + "'>" +
						( showNumbers ? "<div class='fc-day-number'>" + temp + "</div>" : '') +
...
function _renderDaySegs(segs, rowCnt, view, minLeft, maxLeft, getRow, dayContentLeft, dayContentRight, segmentContainer, bindSegHandlers, modifiedEventId) {
	...
	for (i=0; i<segCnt; i++) {
		...
		html +=
			"<div class='" + className + event.className.join(' ') + "' style='position:absolute;z-index:8;left:"+left+"px'>" + (event.url ? " href='" + htmlEscape(event.url) + "'" : '')  +
