<%--
  ...
  - attributes is editable, e.g. is one of Draft or Scheduled statuses
--%>
...
<c:choose>
    <c:when test="${typedContestBrief ne null}">
        <c:if test="${typedContestBrief.software and (statusName eq 'Draft' or statusName eq 'Scheduled')}">
