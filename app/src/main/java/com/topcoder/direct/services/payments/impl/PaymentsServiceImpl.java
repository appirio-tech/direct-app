...
public class PaymentsServiceImpl extends BaseService implements PaymentsService {
    ...
    public Map<Integer, CashOutflowPotentialResult> getPotentialMemberPayments(Date endDate,
        ...
        LoggingWrapperUtility.logEntrance(getLogger(), signature,
                new String[] {"endDate", "paymentMethods"},
                new Object[] {endDate, paymentMethods});
        if (null == endDate) {
            endDate = new Date();
        }
        try {
            Map<Integer, CashOutflowPotentialResult> result = DataProvider.getPotentialMemberPayments(endDate,
                    paymentMethods);
