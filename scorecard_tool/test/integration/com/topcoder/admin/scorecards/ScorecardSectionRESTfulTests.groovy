...
/**
* This class is unit test class for the RESTful service of <code>ScorecardSection</code>
* domain.
*
* @author TCSASSEMBER
* @version 1.0
*/
class ScorecardSectionRESTfulTests extends GroovyTestCase {
    /**
    ...
    */
    void testRESTfulCURD() {
        ...
        assertTrue response.data.success
        assertEquals "test scorecard section", response.data.data.sectionName
        ...
        // GET (list)
        response = client.get(path: "scorecard-section")
        assertTrue response.data.success
        
        // GET (single item)
        response = client.get(path: "scorecard-section/${id}")
        assertTrue response.data.success
        assertEquals "test scorecard section", response.data.data.sectionName
        assertEquals 100.0, response.data.data.weight
        assertEquals 1, response.data.data.sort
        ...
        // PUT
        response = client.put(
            path: "scorecard-section/${id}",
            body: [ data: [sectionName: "test scorecard section2", weight: 100.0, sort: 2, scorecardGroup: scorecardGroupId, dateCreated: new Date(), lastUpdated: new Date() ] ],
            requestContentType : ContentType.JSON.toString()
        )
        assertTrue response.data.success
        assertEquals "test scorecard section2", response.data.data.sectionName
        assertEquals 100.0, response.data.data.weight
        assertEquals 2, response.data.data.sort
        
        // DELETE
        response = client.delete(path: "scorecard-section/${id}")
        assertTrue response.data.success
        assertEquals "test scorecard section2", response.data.data.sectionName
        assertEquals 100.0, response.data.data.weight
        assertEquals 2, response.data.data.sort
